...
const DATA_DIR = process.env.IMAGES2_PROMPT_DATA_DIR || path.join(ROOT_DIR, "data");
const DATA_FILE = path.join(DATA_DIR, "images2-prompt-manager.json");
const PREVIOUS_FILE = path.join(DATA_DIR, "images2-prompt-manager.previous.json");
const TEMP_FILE = path.join(DATA_DIR, "images2-prompt-manager.tmp");
const HOST = "127.0.0.1";
const PORT = Number(process.env.IMAGES2_PROMPT_PORT || 4175);
...
async function handleApi(req, res, pathname) {
  if (pathname === "/api/health" && req.method === "GET") {
    sendJson(res, 200, { app: "images2-prompt-manager", ok: true });
    return;
  }
...
server.listen(PORT, HOST, () => {
  console.log(`Images2 Prompt Manager: http://localhost:${PORT}/`);
